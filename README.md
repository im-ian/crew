...
에이전트 세션 하나를 사람 하나로 봐요. 리뷰는 Claude, 구현은 Grok, 나머지는 Codex처럼 전문가를 만들어 두고, 회사처럼 한 창에서 협업하게 하는 프로그램이에요. 창을 여러 개 띄워 놓고 한 쪽 결과를 다른 쪽에 복사해 넘기던 일을, 다른 전문가에게 일을 맡기는 걸로 바꿔 줘요.
...
이미 쓰는 CLI를 데스크톱 안에 모아 두면:

- 1:1로 한 봇과 대화하거나
- 채널에 여러 봇을 모아 두고 `@이름`으로 부르거나
- 봇이 `crew tell`로 다른 봇에게 직접 일을 넘기거나
...
> 사람에게 고르라고 물으려면 `crew ask --question "..." --option A --option B`를 실행해라. 값을 적으라고 하려면 `--input 아이디`를 보태라. 다른 봇에게 묻지 마라.

그래서 1:1 대화에서 `@리뷰어 이 패치 봐줘`라고 쓰면, 지금 대화 중인 봇이 먼저 일을 하고 필요할 때 리뷰어 봇에게 넘겨요. 봇 사이의 릴레이는 사용자 메시지 하나당 최대 4홉이에요. 이름 변경과 루틴 등록은 채팅에서 시키면 봇이 같은 식으로 `crew`를 실행해요. 사람에게 A / B / C를 고르라고 묻거나 값을 적으라고 할 때도 `crew ask`로 그 봇의 채팅에 카드가 떠요.
...
- 봇이 사람에게 고르거나 값을 적으라고 물으면 (`crew ask`) 그 채팅에 카드가 떠요. 고른 답이나 적은 값이 봇에게 돌아와요. 다른 봇에게 묻지 않아요
...
### 알려진 한계: 데몬 두 개가 동시에 뜰 수 있어요

데몬은 부팅할 때 소켓이 살아 있는지 물어보고, 아니면 남은 소켓 파일을 지우고, 로스터를 열고, 마지막에 `bind` 해요. 두 데몬이 같은 순간에 시작하면 둘 다 "소켓 없음"을 보기 때문에 둘 다 로스터를 열고, `bind` 에서 한쪽이 져요. 검사와 `bind` 가 하나의 연산이 아니라서 생기는 틈이고, 크래시 복구를 위해 남은 소켓을 지워야 하는 한 없앨 수 없어요.

측정하면 피해는 거의 없어요. 실제로 쓰는 봇(`grok`/`claude`/`codex`)은 헤드리스라 부팅이 CLI 프로세스를 띄우지 않고, PTY 봇이라면 진 데몬의 자식들은 그 데몬과 함께 죽어요. 아주 드물게 둘 다 `bind` 에 성공하는 경우는 로스터가 빈 상태에서만 재현되는데, 그때 닿을 수 없게 된 데몬은 아무것도 들고 있지 않아요.

봇을 여러 개 쓰면서 `crew` 명령과 데스크톱 앱을 같은 순간에 처음 실행하는 게 유일한 트리거예요. 한쪽이 뜬 뒤에 다른 쪽을 실행하면 생기지 않아요.
