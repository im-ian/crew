...
export function MdBody({ text, agents, className }: Props) {
  const nodes = useMemo(() => {
    const html = injectMentionChips(renderMarkdown(text), agents);
    return htmlToReact(html, agents);
  }, [text, agents]);
  return <div className={className}>{nodes}</div>;
}

function htmlToReact(html: string, agents: AgentInfo[]): ReactNode[] {
  const tmpl = document.createElement("template");
  tmpl.innerHTML = html;
  return nodesToReact(tmpl.content.childNodes, agents, "m");
}
...
function nodesToReact(
  ...
  Array.from(nodes).forEach((node, i) => {
    const child = nodeToReact(node, agents, prefix + "-" + i);
...
function nodeToReact(
  ...
  if (el.classList.contains("mention-chip")) {
    ...
    if (agent) return <MentionChip key={key} agent={agent} />;
    return id ? <span key={key}>@{id}</span> : null;
  }
  if (tag === "BR") return <br key={key} />;
  const kids = nodesToReact(el.childNodes, agents, key);
